...
def create_app(db_path: str | Path = "dastcore.db") -> FastAPI:
    ...
    async def start_scan(
        ...
        login_url: str = Form(""),
        login_fields: str = Form(""),
        login_headers: str = Form(""),
        login_token_field: str = Form(""),
            ...
            if mode == "ai":
                ...
                scan_id = manager.start(
                    ScanRequest(
                        ...
                        login_fields=[s.strip() for s in login_fields.splitlines() if s.strip()],
                        login_headers=[s.strip() for s in login_headers.splitlines() if s.strip()],
                        login_token_field=login_token_field.strip(),
